...
class MySqlConnection:
    def __init__(self, host, port, user, password, db_name, rebuild_db=False):
        ...
        session = sessionmaker(bind=self.connection.engine, autocommit=True, autoflush=True, enable_baked_queries=False,
                               expire_on_commit=True)  # expire on c. следит за тем, чтобы мы работали с актуальными данными, которые могли прийти из другой локальной сессии по взаимодействию с БД
